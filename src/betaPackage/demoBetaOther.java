...
public class demoBetaOther {
	// Define a static logger variable so that it references the
	// Logger instance named "demoBetaOther" or "MyApp".
	// static final Logger logger = LogManager.getLogger(MyApp.class.getName());
	// ANOTHER ONE: create a logger object for the java class (demoBetaOther) we are coding
	private static Logger myLogger = LogManager.getLogger(demoBetaOther.class.getName());
	//above is the only thing we have to do in order to use the log4j log in our class, just change the className and
	// remember the import in line # 2. With this only the myLogger.error and myLogger.fatal will be displayed in the 
	// console, the others won't be displayed because this java class "demoBethaOther" is NOT defined in 
	// the log4j2.xml configuration file (see lines #10 (log file definition) and 23 (logger definition)).
